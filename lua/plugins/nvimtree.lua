...
local function nvim_tree_trash()
  ...
  local function get_user_input_char()
    ...
    return vim.fn.nr2char(type(c) == 'number' and c or 0)
  end
  ...
  if (get_user_input_char():match('^y') and node) then
    vim.fn.jobstart(trash_cmd .. node.absolute_path, {
      detach = true,
      on_exit = function () api.tree.reload() end,
    })
  end
...
local on_attach = function(bufnr)
  local opts = function(desc)
    return { desc = desc or 'nvim-tree', buffer = bufnr, noremap = true, silent = true, nowait = true }
  end
  ...
  vim.keymap.set('n', 'ga', function()
    ...
    local function git_add(n)
      local gs = n.git_status.file

      -- If the file is untracked, unstaged or partially staged, we stage it
      if gs == "??" or gs == "MM" or gs == "AM" or gs == " M" then
        vim.cmd("silent !git add " .. n.absolute_path)
      -- If the file is staged, we unstage it
      elseif gs == "M " or gs == "A " then
        vim.cmd("silent !git restore --staged " .. n.absolute_path)
      end
  ...
  vim.keymap.set('n', 'gr', function()
    ...
    local function git_restore(n)
      -- If the file is modified, we restore it
      vim.cmd("silent !git restore " .. n.absolute_path)
      api.tree.reload()
    end

    git_restore(node)
  end, opts('Git restore'))
end
